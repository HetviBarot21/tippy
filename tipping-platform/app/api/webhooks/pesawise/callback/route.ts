/**
 * PesaWise STK Push callback webhook handler
 * This endpoint receives payment confirmations from PesaWise
 */
...
import { PesaWiseCallbackData } from '@/utils/pesawise/types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('PesaWise callback received:', JSON.stringify(body, null, 2));
    ...
    if (signature && timestamp) {
      ...
      if (!isValidSignature) {
        console.error('Invalid PesaWise callback signature');
        return NextResponse.json({ error: 'Invalid signature' }, { status: 401 });
      }
    }

    const callbackData: PesaWiseCallbackData = body;
    const { checkout_request_id, merchant_request_id, result_code, result_desc } = callbackData;

    // Parse transaction status
    const status = pesaWiseService.parseTransactionStatus(result_code);

    // Find the tip record using checkout_request_id with restaurant and waiter details
    ...
    if (findError || !tip) {
      console.error('Tip not found for checkout_request_id:', checkout_request_id, findError);
      // Still return success to PesaWise to avoid retries
    ...
    if (status === 'completed') {
      ...
      const transactionDetails = pesaWiseService.extractTransactionDetails(callbackData);
      ...
      if (transactionDetails) {
        ...
        if (updateError) {
          ...
          console.log(`Tip ${tip.id} completed via PesaWise - Amount: KES ${tip.amount}, Commission: KES ${expectedCommissionAmount}, Net: KES ${expectedNetAmount}`);
      ...
      if (updateError) {
        ...
        console.log(`Tip ${tip.id} marked as ${status} via PesaWise: ${result_desc}`);
      }
    }

    // Always return success to PesaWise to avoid retries
    return NextResponse.json({
      success: true,
    ...
    console.error('PesaWise callback processing error:', error);
    
    // Still return success to avoid PesaWise retries for processing errors
    return NextResponse.json({
      success: true,
...
export async function GET() {
  return NextResponse.json({
    message: 'PesaWise callback endpoint is active',
